...
predicted_class = data.traget_+names[prediction[0]]
...
if st.button("Show Decision Tree Diagram"):
    ...
    tree.plot_tree(clf, feature_names=data.feature_nmaes, class_names=data=data.target_names, filled=True, ax=ax)
    buf = io.BytesIO()             
    plt.savefig(buf, format="png")
    buf.seek(0)
    st.image(buf, captain="Decision Tree Visualization", use_column_width=True)
